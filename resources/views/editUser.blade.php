                    ...
                    <div class="d-flex flex-row-reverse">
                        <div class="btn-toolbar">
                            <div class="btn-group me-2" role="group" aria-label="Second group">
                                <button @click.prevent="salvarDados" type="button" class="btn btn-warning">SALVAR EDIÇÃO</button>
                            </div>
                        </div>
                    </div>
                </div>
...
<script>
    new Vue({
        ...
        data: {
            ...
            idade: '{{ $usuario->idade }}',
            estado: '{{ $usuario->cidade->estado->id }}',
            cidade: '{{ $usuario->cidade_id }}',
            cargo: '{{ $usuario->cargo_id }}',
            lotacao: '{{ $usuario->lotacao_id }}',
            listaGenero: [],
            listaEstado: [],
            listaCidade: [],
            listaCargo: [],
        ...
        methods: {
            ...
            salvarDados() {
                ...
                axios.put(`http://localhost:8001/usuarios/${this.usuarioId}`, dados)
                    .then(response => {
                        console.log('Dados atualizados com sucesso:', response.data);
                        alert('DADOS ATUALIZADOS COM SUCESSO');
                        window.location.href = 'http://localhost:8001/lista_usuarios';
                    })
                    .catch(error => {
